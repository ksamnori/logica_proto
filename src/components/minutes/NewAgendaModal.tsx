  ...
  const handleSubmit = async () => {
    ...
    try {
      await supabase.from("agenda").insert({
        ...
        // 비밀 안건일 경우 식별자 추가
        is_secret: isSecret,
        attendees: isSecret ? currentUser.name : null // 수동 안건은 본인만 열람하게 됨 (병합 시 변경 가능)
      });
      alert("새 안건이 상정되었습니다.");
      onSuccess();
    } catch (e) { alert("저장 실패"); }
        ...
        <div className="p-5 space-y-4 bg-slate-50 flex-1">
          <div className="flex items-end gap-3">
            ...
            <div className="shrink-0">
              <button 
                onClick={() => alert("녹음 모달로 연결하거나 녹음 로직을 시작합니다. (추가 구현 필요)")}
                className="h-[38px] px-3 bg-white text-blue-600 border border-blue-200 rounded-lg text-[11px] font-bold shadow-sm hover:bg-blue-50 transition-colors flex items-center gap-1"
              >
                🎙️ 음성 녹음
              </button>
            </div>
