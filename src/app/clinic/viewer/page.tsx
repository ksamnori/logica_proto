...
export default function ClinicViewer() {
  ...
  const handleTimeUp = async (forceAction?: string, sessionExpired = false) => {
    ...
    if (isTimedRound && !forceAction) {
      questions.forEach((qItem, i) => {
        ...
        reviewList.push({
          idx: i, uid: qItem.uid, qNum: i + 1, questionText: qItem.questionText, correctAnswer: qItem.answer, imageDataUrl: studentDrawings.current[i] || null,
          recognizedText: meta?.recognized_text || '', aiExplanation: meta?.explanation || '', aiConfidence: meta?.confidence ?? null, recordId, tqId: qItem.tq_id ?? null, questionId: qItem.question_id ?? null,
          examAssignmentId: qItem.question_id && params.assignmentId ? params.assignmentId : null, requested: false, resolved: false,
        });
      });
    }
  ...
  const processCorrectAnswer = async (qItem: any, idx: number, fromRecheck: boolean) => {
    ...
    if (nextIdx === null) {
      ...
      questions.forEach((qi, i) => { const st = qBoxStatus.current[i]; if (st === 'wrong_red' && qi.question_id) { incorrectQIds.push(qi.question_id); statusMap[qi.question_id] = 'X'; } });
  ...
  if (!isStarted) {
    let remainCount = 0; questions.forEach((_, i) => { if (!qBoxStatus.current[i]) remainCount++; });
    return (
          ...
          <div className="w-full max-w-[1920px] grid grid-cols-[65fr_35fr] gap-6 md:gap-8 h-full relative">
            ...
            <div className="flex flex-col gap-5 min-h-0 overflow-y-auto custom-scrollbar">
              ...
              <ClinicQuestionNav 
                ...
                qBoxStatus={qBoxStatus} studentAnswers={studentAnswers} callState={callState}
