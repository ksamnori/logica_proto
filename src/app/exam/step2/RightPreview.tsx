      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 pb-32 space-y-6 scroll-smooth" id="right-problem-list">
        {isLoading ? <div className="text-center font-bold text-slate-500 mt-20 flex flex-col items-center"><svg className="animate-spin h-12 w-12 text-[#002864] mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><p>DB에서 문제를 추출 중입니다...</p></div> : 
         questions.map((g: any, idx: number) => {
           ...
           return (
             <div key={g.id} id={`problem-card-${idx}`} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-row group transition-all duration-300 hover:border-blue-400">
               <div className="w-[64px] bg-slate-50 flex flex-col items-center py-5 shrink-0 gap-1.5 border-r border-slate-200 relative">
                 <div draggable onDragStart={(e) => handleDragStart(e, idx)} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, idx)} className="absolute top-0 left-0 w-full h-4 bg-slate-200 hover:bg-blue-300 cursor-grab opacity-0 group-hover:opacity-100 flex justify-center items-center transition-opacity" title="드래그하여 순서 변경">
                   <svg className="w-3 h-3 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 9h8M8 15h8"></path></svg>
                 </div>
               ...
               <div className="flex-1 flex flex-col relative p-5 min-w-0">
                 <div className="flex justify-between items-center mb-2 border-b border-slate-100 pb-1">
                   <span className="text-[14px] font-bold text-slate-600">{depth6Name}</span>
                 </div>
                   ...
                   return (
                     <div key={q.question_id} className={`relative ${subIdx < g.items.length - 1 ? 'mb-8 pb-8 border-b-2 border-dashed border-slate-200' : ''}`}>
                       <div className="flex justify-between items-start mb-1">
                         ...
                         <div className="flex gap-1.5 shrink-0 z-10 relative ml-2">
                       ...
                       {!isEditing ? (
                         <>
                           <div className="font-myungjo font-semibold text-[16px] text-slate-800 leading-[2.2] tracking-wide break-keep" dangerouslySetInnerHTML={{ __html: formatText(q.question) }} />
                           {q.image_url && <img src={getCleanUrl(q.image_url)} className="max-w-full object-contain my-4 mix-blend-multiply rounded border border-slate-200" style={{ maxHeight: '250px' }} alt="" />}
                           
                           {showAnswer && (
                             <div className="mt-6 pt-5 border-t border-dashed border-slate-200 bg-slate-50 p-4 rounded-lg">
                               ...
                               {q.answer_image_url && <img src={getCleanUrl(q.answer_image_url)} className="max-w-xs mt-3 rounded border border-slate-200" alt="" />}
                         ...
                         <div className="p-5 bg-amber-50 rounded-xl border border-amber-200 shadow-inner">
                           <label className="block text-[12px] font-extrabold text-amber-800 mb-1">📝 문제 수정</label>
                           <textarea value={editForm.question} onChange={e => setEditForm({...editForm, question: e.target.value})} className="w-full p-3 border border-amber-300 rounded mb-3 text-[14px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={3}></textarea>
                           
                           <label className="block text-[12px] font-extrabold text-amber-800 mb-1">🎯 정답 수정</label>
                           <textarea value={editForm.answer} onChange={e => setEditForm({...editForm, answer: e.target.value})} className="w-full p-2 border border-amber-300 rounded mb-3 text-[14px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={1}></textarea>
                           
                           <label className="block text-[12px] font-extrabold text-amber-800 mb-1 mt-2">📖 일반 해설 및 풀이</label>
                           <div className="flex gap-2 mb-3">
                               <textarea value={editForm.explanation} onChange={e => setEditForm({...editForm, explanation: e.target.value})} className="w-1/2 p-2 border border-amber-300 rounded text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={2} placeholder="일반 해설"></textarea>
                               <textarea value={editForm.solution} onChange={e => setEditForm({...editForm, solution: e.target.value})} className="w-1/2 p-2 border border-amber-300 rounded text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={2} placeholder="일반 풀이"></textarea>
                           </div>

                           <label className="block text-[12px] font-extrabold text-amber-800 mb-1 mt-2">🔍 단계별 해설 (스텝 1~4)</label>
                           <div className="grid grid-cols-2 gap-2 mb-3">
                               <textarea value={editForm.step_1_concept} onChange={e => setEditForm({...editForm, step_1_concept: e.target.value})} className="w-full p-2 border border-amber-300 rounded text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={2} placeholder="[스텝 1: 개념 및 접근법]"></textarea>
                               <textarea value={editForm.step_2_approach} onChange={e => setEditForm({...editForm, step_2_approach: e.target.value})} className="w-full p-2 border border-amber-300 rounded text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={2} placeholder="[스텝 2: 풀이 전략]"></textarea>
                               <textarea value={editForm.step_3_process} onChange={e => setEditForm({...editForm, step_3_process: e.target.value})} className="w-full p-2 border border-amber-300 rounded text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={2} placeholder="[스텝 3: 풀이 과정]"></textarea>
                               <textarea value={editForm.step_4_conclusion} onChange={e => setEditForm({...editForm, step_4_conclusion: e.target.value})} className="w-full p-2 border border-amber-300 rounded text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-amber-500 custom-scrollbar" rows={2} placeholder="[스텝 4: 결론]"></textarea>
                           </div>

                           <div className="flex justify-end gap-2 mt-4">
                               <button onClick={() => setEditingId(null)} className="px-4 py-2 bg-white border border-slate-300 text-slate-600 font-bold text-xs rounded hover:bg-slate-50 shadow-sm">취소</button>
                               <button onClick={saveEdit} className="px-4 py-2 bg-[#002864] text-white font-bold text-xs rounded hover:bg-blue-900 shadow-sm">💾 각각 저장</button>
                           </div>
                         </div>
                       )}
