...
export const getAllIds = (node: any) => {
  ...
  const traverse = (n: any) => {
    ...
    if (n.children) {
      ...
      for (const k in n) {
        if (k !== 'itemId' && k !== 'categoryId') traverse(n[k]);
      }
    }
  };
  traverse(node);
...
const TreeNode = React.memo(({ nodeKey, node, isRoot, selectedItemIds, toggleItem, toggleFolder }: any) => {
  ...
  if (node.children) {
    ...
    return (
      <details open={isOpen} onToggle={(e: any) => setIsOpen(e.currentTarget.open)} className={isRoot ? "mb-2 pl-2 border-l-2 border-slate-100 ml-2" : "mb-1 pl-2 border-l border-slate-100 ml-3"}>
        ...
        {isOpen && (
          <div className="ml-4 mt-1 space-y-1 border-l border-slate-100 pl-2">
            {Object.keys(node.children).sort().map(k => (
              <TreeNode key={k} nodeKey={k} node={node.children[k]} isRoot={false} selectedItemIds={selectedItemIds} toggleItem={toggleItem} toggleFolder={toggleFolder} />
            ))}
          </div>
        )}
      </details>
  ...
  const memoizedTree = useMemo(() => {
    ...
    return (
      <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
        {Object.keys(targetData).sort().map(k => (
          <TreeNode key={k} nodeKey={k} node={targetData[k]} isRoot={true} selectedItemIds={selectedItemIds} toggleItem={toggleItem} toggleFolder={toggleFolder} />
        ))}
      </div>
    );
        ...
        <>
          <div className="shrink-0 border-b border-slate-200 bg-white">
            <div className="flex px-4 pt-4 space-x-5 border-b border-slate-100 overflow-x-auto whitespace-nowrap no-scrollbar">
              {currentMode === 'regular' && Object.keys(masterData).sort().map(d1 => (
              ...
              {currentMode === 'thinking' && Object.keys(thinkingData).sort().map(d1 => (
                <button key={d1} onClick={() => updateD1(d1)} className={`pb-3 px-2 text-base font-bold border-b-4 transition-colors ${currentD1 === d1 ? 'border-[#002864] text-[#002864]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>{d1}</button>
              ))}
              {currentMode === 'test' && Object.keys(TEST_DATA).sort().map(d1 => (
                <button key={d1} onClick={() => updateD1(d1)} className={`pb-3 px-2 text-base font-bold border-b-4 transition-colors ${currentD1 === d1 ? 'border-[#002864] text-[#002864]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>{d1}</button>
              ))}
            </div>
            {currentMode === 'regular' && (
              <div className="flex px-4 py-3 gap-2 flex-wrap">
                {Object.keys(masterData[currentD1] || {}).sort().map(d2 => (
