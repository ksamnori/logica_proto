            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                filteredExams.map(exam => {
                  // 💡 예외 처리: 데이터 형태가 다르거나 비어있어도 화면이 터지지 않도록 방어 코드 추가
                  const instructor = Array.isArray(exam.instructor) ? exam.instructor[0] : exam.instructor;
                  ...
                  // 💡 Hydration Error 방지: 날짜 계산을 렌더링 시점에 직접 하지 않고, 안전하게 서버 DB 시간을 기준으로 가공
                    ...
                    <tr key={exam.exam_id} className="hover:bg-blue-50/30 transition-colors border-b border-slate-100">
                      ...
                      <td className="py-4 px-5">
                        <div className="flex items-center justify-end gap-1.5 whitespace-nowrap">
                          ...
                          {isDistributed ? (
                            <button disabled className="w-[60px] shrink-0 py-1.5 bg-slate-50 text-slate-400 rounded text-[11px] font-bold cursor-not-allowed border border-slate-200 text-center px-0">수정불가</button>
                          ) : (
