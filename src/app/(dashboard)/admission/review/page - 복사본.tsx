...
function ReviewContent() {
  ...
  // 💡 핵심 1: isAdmission(입학테스트 여부)을 받아서 admission_ratio를 적용하는 로직 추가
  const calcScoreRatio = (code: string, assignedScore: number, metaMap: any, isAdmission: boolean = false) => {
    ...
    if (meta) {
      // 입학테스트일 경우 새로 만든 admission_ratio 참조, 아니면 기존 score_ratio 참조
      ...
      // 입학테스트면 0.0점 초과면 O, 0점이면 X로 내부 인식 (채점결과 색상 등에 영향)
      if (isAdmission) {
        isCorrectEq = ratio > 0;
      } else {
      ...
      groups.forEach((g: any) => {
        ...
        g.items.forEach((i: any) => {
          ...
          if (i.answer?.grading_code) {
            ...
            // 과제는 입학테스트가 아니므로 isAdmission = false
      ...
      groups.forEach((g: any, index: number) => {
        ...
        g.items.forEach((i: any) => {
          ...
          if (i.answer?.grading_code) {
            ...
            // 💡 핵심 3: 이 시험지가 입학테스트인지 판별해서 calcScoreRatio에 넘김
  ...
  const updateGradingCodeLocally = (answerId: string, code: string, questionId: string, tqId: number) => {
    ...
    // 💡 버튼 누를 때마다 실시간 점수 반영 시에도 입학테스트 여부 넘김
