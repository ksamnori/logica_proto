...
function TabInitHandler({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  ...
  useEffect(() => { if (tabParam) setActiveTab(tabParam); }, [tabParam, setActiveTab]);
  return null;
}

// ---------------------------------------------------------
// 2. 메인 페이지 컴포넌트 (데이터 로딩 및 상태 관리)
// ---------------------------------------------------------
export default function StudentDetailPage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (!stuData) { setIsNotFound(true); return; }
      setStudent(stuData);
      ...
      if (enrollData && enrollData.length > 0) loadProgressData(enrollData.map(e => e.class_id));
    } catch (error) { console.error("로딩 에러:", error); }
  };

  const loadClasses = async () => {
    const tId = localStorage.getItem("logica_tenant_id");
    let query = supabase.from("class").select("class_id, code, name, level_name, instructor(name)").neq("status", "종료").neq("status", "폐강");
  ...
  const loadConsultLogs = async () => {
    ...
    const formattedAdmissionLogs = (admissionLogs || []).filter((app: any) => app.counseling_memo && app.counseling_memo.trim() !== "").map((app: any) => ({
      consultation_log_id: `admission_${app.application_id}`,
      consultation_type: "입학 상담", contact_method: "방문/테스트",
      content: `[결과: ${app.test_result}]\n${app.counseling_memo}`, created_at: app.created_at, instructor: { name: "입학 담당" }, is_admission: true
    }));

    const mappedRegularLogs = (regularLogs || []).map((log: any) => ({
      ...log, id: log.consultation_log_id, log_id: log.consultation_log_id
    }));
  ...
  const loadProgressData = async (classIds: string[]) => {
    ...
    const resultMap = new Map(results?.map((r: any) => [r.homework_id, r]));
    ...
    const books = cbData.map((cb: any) => {
       const bQs = qData?.filter((q: any) => q.book_id === cb.book_id) || [];
  ...
  const loadExamResults = async () => {
    try {
      ...
      allAnswers.forEach(ans => {
         if(!assignCounts[ans.exam_assignment_id]) {
           assignCounts[ans.exam_assignment_id] = { o:0, ro:0, to:0, totalAns:0, x:0 };
         }
  ...
  const loadCategoryAnalysis = async () => {
    try {
      ...
      const { data: answers, error: ansErr } = await supabase
        .from('student_answer')
        .select('question_id, is_correct, grading_code')
        .in('exam_assignment_id', targetAssignIds);
      ...
      for (let i = 0; i < qIds.length; i += 200) {
        ...
        const { data: qData } = await supabase
          .from('question_db')
          .select('question_id, taxonomy_id')
      ...
      await Promise.all(uniqueD4Array.map(async (code) => {
        try {
          const { data, error } = await supabase
            ...
            .select('category_id, depth4')
            .eq('category_id', code)
            .limit(1);
            
          if (data && data.length > 0 && data[0].depth4) {
            catMap.set(code, data[0].depth4);
            return;
          }
          ...
          const { data: likeData, error: likeErr } = await supabase
            .from('master_category')
            .select('category_id, depth4')
            .like('category_id', `${code}-%`)
            .limit(1);
            
          if (likeData && likeData.length > 0 && likeData[0].depth4) {
            catMap.set(code, likeData[0].depth4);
          }
        } catch (e) {
          console.error("단원명 역추적 실패:", code, e);
        }
      }));
      ...
      answers.forEach(a => {
        ...
        if (a.grading_code === 'O') {
          obj.initCorrect += 1;
          obj.finCorrect += 1;
        } else if (a.grading_code === 'RO' || a.grading_code === 'TO') {
          obj.finCorrect += 1;
        } else if (!a.grading_code && a.is_correct) {
          obj.initCorrect += 1;
          obj.finCorrect += 1;
        }
      });
      ...
      console.error("단원별 성취도 역추적 로드 실패:", err);
      setCategoryAnalysis([]);
    }
  };

  const loadHwList = async () => {
    const { data } = await supabase.from("student_homework_result").select("status, completed_tq_ids, checked_at, homework_assignment(homework_title, target_questions, created_at, due_date, textbook(title, book_type))").eq("student_id", studentId).order("checked_at", { ascending: false });
    setHwList(data || []);
  };

  const loadClinicList = async () => {
    const { data } = await supabase.from("clinic_task").select("*, textbook_question(question_number)").eq("student_id", studentId).order("created_at", { ascending: false });
    setClinicList(data || []);
  };
  ...
  const handleSaveSchoolExam = async (e: React.FormEvent) => {
    e.preventDefault(); setIsSavingSchoolExam(true);
    try {
      const tId = localStorage.getItem("logica_tenant_id");
      await supabase.from("student_school_exam").insert({ student_id: studentId, year: schoolExamForm.year, semester: schoolExamForm.semester, examType: schoolExamForm.examType, subject: schoolExamForm.subject, score: parseFloat(schoolExamForm.score), tenant_id: tId });
      alert("학교 내신 성적이 등록되었습니다.");
      setIsSchoolExamModalOpen(false); setSchoolExamForm({ ...schoolExamForm, score: "" }); loadSchoolExams();
    } catch (error: any) { alert("저장 실패"); } finally { setIsSavingSchoolExam(false); }
  };

  const deleteSchoolExam = async (id: string) => {
    if (!confirm("삭제하시겠습니까?")) return;
    await supabase.from("student_school_exam").delete().eq("id", id); loadSchoolExams();
  };

  const handleReEnroll = async (e: React.FormEvent) => {
    e.preventDefault(); setIsSubmittingReEnroll(true);
    try {
      const tId = localStorage.getItem("logica_tenant_id");
      ...
      alert("재등록 처리 완료"); setIsReEnrollModalOpen(false); loadInitialData(); 
  ...
  const toIsoString = (timeStr: string, dateStr: string) => {
    if (!timeStr) return null; const [hh, mm] = timeStr.split(':');
  ...
  const saveAttendance = async () => {
    ...
    try {
      ...
      const payload = { student_id: studentId, class_id: classId, enrollment_id: enrollId, attendance_date: selectedDate, status: attendForm.status, check_in_time: toIsoString(attendForm.checkIn, selectedDate), check_out_time: toIsoString(attendForm.checkOut, selectedDate), remark: attendForm.remark };
      ...
      alert("출결 기록이 저장되었습니다."); loadAttendance();
    } catch (e) { alert("저장 실패"); }
  };

  const deleteAttendance = async () => {
    if (!attendForm.id || !confirm("해당 날짜의 출결 기록을 삭제하시겠습니까?")) return;
    try {
      await supabase.from('attendance').delete().eq('attendance_id', attendForm.id);
      alert("기록이 삭제되었습니다."); setAttendForm({ id: "", status: "출석", checkIn: "", checkOut: "", remark: "" }); loadAttendance();
    } catch (e) { alert("삭제 실패"); }
  };
  ...
  const calendarDays = useMemo(() => {
    ...
    const days = Array.from({ length: daysInMonth }).map((_, i) => {
      const d = i + 1; const dateStr = `${calYear}-${String(calMonth+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
      const status = attendMap[dateStr]; const hasConsult = consultMap[dateStr]; const isActive = dateStr === selectedDate;
      let dotColor = "bg-transparent";
      if (status === "출석") dotColor = "bg-[#10b981]"; else if (status === "지각") dotColor = "bg-[#eab308]";
      else if (status === "조퇴") dotColor = "bg-[#f97316]"; else if (status === "결석") dotColor = "bg-[#ef4444]";
      const bgClass = isActive ? 'bg-[#002864] text-white border-[#002864] shadow-md' : (hasConsult ? 'bg-blue-50 border-blue-200 text-blue-800 hover:bg-blue-100' : 'bg-white border-slate-100 hover:border-slate-300 text-slate-700');
      
      return (
        <div key={d} onClick={() => setSelectedDate(dateStr)} className={`relative aspect-square rounded-lg flex flex-col items-center justify-center font-black text-[12px] cursor-pointer transition-all border ${bgClass}`}>
          {hasConsult && <span className="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 border-[1.5px] border-white rounded-full shadow-sm z-10" title="상담 기록 있음"></span>}
  ...
  const attSummary = useMemo(() => {
    let curr = { total: 0, present: 0, leave: 0, absent: 0 }, prev = { total: 0, present: 0, leave: 0, absent: 0 };
  ...
  const renderCalendarBlock = () => (
    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm w-full flex flex-col items-center">
      <div className="w-full max-w-[260px]">
        ...
        <div className="grid grid-cols-7 gap-1 text-slate-400 text-[10px] text-center mb-2 font-bold"><div>일</div><div>월</div><div>화</div><div>수</div><div>목</div><div>금</div><div>토</div></div>
        <div className="grid grid-cols-7 gap-1.5 mb-4">{calendarDays}</div>
  ...
  if (isNotFound) return (
    <div className="flex-1 flex flex-col items-center justify-center min-h-[80vh] bg-slate-50 p-10 font-pretendard">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-center max-w-md w-full animate-[fadeIn_0.3s_ease-out]">
        <div className="text-5xl mb-4">👻</div><h2 className="text-xl font-black text-slate-800 mb-2">학생을 찾을 수 없습니다.</h2>
        <button onClick={() => router.back()} className="px-6 py-3 bg-[#002864] text-white font-bold rounded-lg shadow-sm hover:bg-blue-900 transition-colors w-full mt-4">이전 화면으로 돌아가기</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-slate-50 h-full relative overflow-y-auto custom-scroll p-4 pt-20 sm:p-8 sm:pt-24 font-pretendard">
      <Suspense fallback={null}><TabInitHandler setActiveTab={setActiveTab} /></Suspense>
      ...
      <div className="max-w-[1300px] w-full mx-auto space-y-5 pb-20">
        ...
        <div className="flex justify-between items-start bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10 flex items-center gap-2">
            <button onClick={() => router.back()} className="px-3 py-2 bg-white text-slate-600 font-bold rounded-lg shadow-sm border border-slate-200 hover:bg-slate-50 hover:text-slate-800 transition-colors text-xs mr-2">← 목록으로</button>
            {(student.status === "휴원" || student.status === "퇴원") && (
              <button onClick={() => setIsReEnrollModalOpen(true)} className="px-4 py-2 bg-emerald-600 text-white font-bold rounded-lg shadow-sm hover:bg-emerald-700 text-xs animate-pulse">🔄 복귀/재등록</button>
            )}
            <button onClick={() => setIsEditModalOpen(true)} className="px-4 py-2 bg-[#002864] text-white font-bold rounded-lg shadow-sm hover:bg-blue-900 text-xs">정보 수정</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
          <div className="flex overflow-x-auto border-b border-slate-200 bg-slate-50/80 px-2 pt-2 custom-scroll shrink-0">
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-5 py-3 font-black text-[12px] whitespace-nowrap border-b-2 transition-colors ${activeTab === tab.id ? 'text-[#002864] border-[#002864]' : 'text-slate-400 border-transparent hover:text-slate-700 hover:bg-slate-100/50'}`}>
                {tab.name}
              </button>
            ))}
          </div>
      ...
      <StudentEditModal isOpen={isEditModalOpen} studentId={studentId} student={student} enrollments={enrollments} allClasses={allClasses} onClose={() => setIsEditModalOpen(false)} onSuccess={loadInitialData} />
      <ConsultModal isOpen={isConsultModalOpen} studentId={studentId} instId={currentUser.instId} logData={selectedConsultLog} onClose={() => setIsConsultModalOpen(false)} onSuccess={loadConsultLogs} />
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]">
            ...
            <form onSubmit={handleSaveSchoolExam} className="p-5 bg-slate-50 flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-bold text-slate-500 mb-1">연도</label><input type="number" value={schoolExamForm.year} onChange={(e: any) => setSchoolExamForm({...schoolExamForm, year: parseInt(e.target.value)})} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 font-bold focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm" /></div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1">학기</label>
                  <select value={schoolExamForm.semester} onChange={(e: any) => setSchoolExamForm({...schoolExamForm, semester: parseInt(e.target.value)})} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 font-bold focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-white text-sm"><option value={1}>1학기</option><option value={2}>2학기</option></select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1">고사</label>
                  <select value={schoolExamForm.examType} onChange={(e: any) => setSchoolExamForm({...schoolExamForm, examType: e.target.value})} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 font-bold focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-white text-sm"><option value="중간고사">중간고사</option><option value="기말고사">기말고사</option><option value="수행평가">수행평가</option><option value="모의고사">모의고사</option></select>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1">과목</label>
                  <select value={schoolExamForm.subject} onChange={(e: any) => setSchoolExamForm({...schoolExamForm, subject: e.target.value})} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-300 font-bold focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-white text-sm"><option value="수학">수학</option><option value="영어">영어</option><option value="국어">국어</option><option value="과학">과학</option><option value="사회">사회</option><option value="기타">기타</option></select>
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1">취득 점수</label>
                <div className="relative">
                  <input type="number" step="0.1" required value={schoolExamForm.score} onChange={(e: any) => setSchoolExamForm({...schoolExamForm, score: e.target.value})} className="w-full px-2.5 py-2 rounded-lg border border-slate-300 font-black text-indigo-700 text-base focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="예: 95.5" />
              ...
              <div className="mt-2 flex gap-2">
                ...
                <button type="submit" disabled={isSavingSchoolExam} className="flex-1 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-sm text-xs">{isSavingSchoolExam ? "저장 중..." : "등록"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]">
            ...
            <form onSubmit={handleReEnroll} className="p-5 bg-slate-50 flex flex-col gap-3">
              <div className="text-[11px] font-bold text-slate-600 mb-1 leading-relaxed"><span className="text-emerald-600">[{student?.name}]</span> 학생을 다시 재원생으로 전환하고 새로운 반에 배정합니다.</div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1">배정할 수강 반 <span className="text-rose-500">*</span></label>
                <select required value={reEnrollForm.classId} onChange={(e: any) => setReEnrollForm({...reEnrollForm, classId: e.target.value})} className="w-full px-2.5 py-2 rounded-lg border border-slate-300 font-bold text-slate-700 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 bg-white text-sm">
                  <option value="" disabled>반을 선택해주세요</option>
                  {allClasses.map(c => <option key={c.class_id} value={c.class_id}>{c.code?.replace(/\d{4}$/, '').trim() || c.name} ({c.instructor?.name || '미정'})</option>)}
                </select>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1">수강 시작일 (재등록일) <span className="text-rose-500">*</span></label>
                <input type="date" required value={reEnrollForm.startDate} onChange={(e: any) => setReEnrollForm({...reEnrollForm, startDate: e.target.value})} className="w-full px-2.5 py-2 rounded-lg border border-slate-300 font-bold text-slate-700 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 bg-white text-sm" />
              </div>
              <div className="mt-2 flex gap-2">
                ...
                <button type="submit" disabled={isSubmittingReEnroll} className="flex-1 py-2 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-sm text-xs">{isSubmittingReEnroll ? "처리 중..." : "재등록 완료"}</button>
              </div>
            </form>
          </div>
        </div>
...
function ConsultTab({ consultLogs, setSelectedConsultLog, setIsConsultModalOpen, deleteConsultLog, calendarBlock }: any) {
  return (
    <div className="flex flex-col lg:flex-row gap-6 animate-[fadeIn_0.3s_ease-out] h-full">
      <div className="flex-1 min-w-0 flex flex-col h-full">
        <div className="flex justify-between items-center mb-2 border-b border-slate-100 pb-4 shrink-0">
          ...
          <button onClick={() => { setSelectedConsultLog(null); setIsConsultModalOpen(true); }} className="px-4 py-2 bg-[#002864] text-white font-bold rounded-lg text-[11px] shadow-sm hover:bg-blue-900 transition-colors flex items-center gap-1">+ 새 상담 기록</button>
        </div>
          ...
          {consultLogs.length === 0 ? <div className="text-center py-10 text-slate-400 font-bold border border-slate-200 rounded-xl bg-slate-50 text-xs">등록된 상담 기록이 없습니다.</div> : 
            consultLogs.map((log: any, idx: number) => {
              ...
              if (log.is_admission) {
                ...
                switch(log.consultation_type) {
                  case '퇴원상담': badgeCol = 'bg-rose-50 text-rose-600 border-rose-200'; lineCol = 'bg-rose-400'; break;
                  case '신규상담': badgeCol = 'bg-emerald-50 text-emerald-600 border-emerald-200'; lineCol = 'bg-emerald-400'; break;
                  case '성적상담': badgeCol = 'bg-violet-50 text-violet-600 border-violet-200'; lineCol = 'bg-violet-400'; break;
                  case '태도상담': badgeCol = 'bg-amber-50 text-amber-600 border-amber-200'; lineCol = 'bg-amber-400'; break;
                }
              }

              return (
                <div key={log.id || log.log_id || idx} className={`bg-white p-4 rounded-xl border shadow-sm relative overflow-hidden group ${borderCol}`}>
                  <div className={`absolute top-0 left-0 w-1 h-full ${lineCol}`}></div>
                  <div className="flex justify-between items-start mb-2 pl-2">
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded border ${badgeCol}`}>{log.contact_method}</span>
                      ...
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 ml-1">
                        {!log.is_admission && (
                          <>
                            ...
                            <button onClick={() => deleteConsultLog(log.id || log.log_id)} className="text-[10px] text-slate-400 hover:text-rose-600 font-bold px-1.5 rounded bg-slate-50 hover:bg-rose-50 transition-colors">삭제</button>
                          </>
                        )}
                      </div>
                    </div>
      ...
      <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-4">{calendarBlock}</div>
    </div>
  );
}

function AttendTab({ calMonth, attSummary, selectedDate, attendForm, setAttendForm, allowedActions, deleteAttendance, saveAttendance, calendarBlock }: any) {
  return (
    <div className="flex flex-col lg:flex-row gap-6 animate-[fadeIn_0.3s_ease-out] h-full">
          ...
          <div className="flex flex-col xl:flex-row gap-4">
            <div className="flex-1 opacity-70">
              ...
              <div className="grid grid-cols-4 gap-2">
                <div className="bg-white border border-slate-200 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-slate-500 mb-0.5">전체</div><div className="text-base font-black text-slate-800">{attSummary.prev.total}</div></div>
                <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-emerald-600 mb-0.5">출석/지각</div><div className="text-base font-black text-emerald-700">{attSummary.prev.present}</div></div>
                <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-indigo-600 mb-0.5">조퇴</div><div className="text-base font-black text-indigo-700">{attSummary.prev.leave}</div></div>
                <div className="bg-rose-50 border border-rose-100 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-rose-600 mb-0.5">결석</div><div className="text-base font-black text-rose-700">{attSummary.prev.absent}</div></div>
              </div>
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-4 gap-2">
                <div className="bg-white border border-slate-200 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-slate-500 mb-0.5">전체</div><div className="text-base font-black text-slate-800">{attSummary.curr.total}</div></div>
                <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-emerald-600 mb-0.5">출석/지각</div><div className="text-base font-black text-emerald-700">{attSummary.curr.present}</div></div>
                <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-indigo-600 mb-0.5">조퇴</div><div className="text-base font-black text-indigo-700">{attSummary.curr.leave}</div></div>
                <div className="bg-rose-50 border border-rose-100 rounded-lg p-2.5 text-center shadow-sm"><div className="text-[10px] font-bold text-rose-600 mb-0.5">결석</div><div className="text-base font-black text-rose-700">{attSummary.curr.absent}</div></div>
              </div>
            </div>
          </div>
        </div>
          ...
          {selectedDate ? (
            <div className="flex flex-col h-full">
              <div className="flex justify-between items-center mb-4 border-b border-slate-200 pb-3">
                 ...
                 {attendForm.id && allowedActions.includes('action_edit_attend') && <button onClick={deleteAttendance} className="text-[10px] text-rose-500 font-bold hover:underline">기록 삭제</button>}
              </div>
              <div className="space-y-4 flex-1">
                {allowedActions.includes('action_edit_attend') ? (
                  <>
                    <div><label className="block text-[11px] font-bold text-slate-600 mb-1.5">출결 상태</label><select value={attendForm.status} onChange={(e: any) => setAttendForm({...attendForm, status: e.target.value})} className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[12px] font-bold text-slate-800 focus:border-[#002864] focus:outline-none"><option value="출석">🟢 출석</option><option value="지각">🟡 지각</option><option value="조퇴">🟠 조퇴</option><option value="결석">🔴 결석</option></select></div>
                    <div className="grid grid-cols-2 gap-3">
                      <div><label className="block text-[11px] font-bold text-slate-600 mb-1.5">등원 시간</label><input type="time" value={attendForm.checkIn} onChange={(e: any) => setAttendForm({...attendForm, checkIn: e.target.value})} disabled={attendForm.status === '결석'} className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[12px] font-bold text-slate-800 focus:border-[#002864] focus:outline-none disabled:opacity-50" /></div>
                      <div><label className="block text-[11px] font-bold text-slate-600 mb-1.5">하원 시간</label><input type="time" value={attendForm.checkOut} onChange={(e: any) => setAttendForm({...attendForm, checkOut: e.target.value})} disabled={attendForm.status === '결석'} className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[12px] font-bold text-slate-800 focus:border-[#002864] focus:outline-none disabled:opacity-50" /></div>
                    </div>
                    <div><label className="block text-[11px] font-bold text-slate-600 mb-1.5">비고 (선생님 메모)</label><textarea rows={2} value={attendForm.remark} onChange={(e: any) => setAttendForm({...attendForm, remark: e.target.value})} placeholder="예: 병원 진료 후 늦게 등원함" className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[12px] focus:border-[#002864] focus:outline-none resize-none"></textarea></div>
                  </>
                ) : <div className="flex-1 flex flex-col items-center justify-center text-slate-400 font-bold text-xs pt-10">수정 권한이 없습니다.</div>}
              </div>
              {allowedActions.includes('action_edit_attend') && <div className="mt-4 shrink-0"><button onClick={saveAttendance} className="w-full py-2.5 bg-[#002864] text-white font-bold rounded-lg shadow-sm hover:bg-blue-900 transition-colors text-[12px]">기록 저장</button></div>}
            </div>
          ) : <div className="flex-1 flex flex-col items-center justify-center text-slate-400 font-bold text-xs py-20">우측 달력에서 날짜를 클릭하세요.</div>}
        </div>
      </div>
      <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-4">{calendarBlock}</div>
    </div>
...
function ProgressTab({ progressBooks }: any) {
  const active = progressBooks.filter((b: any) => b.status !== '완료');
  const completed = progressBooks.filter((b: any) => b.status === '완료');
  return (
    <div className="flex flex-col lg:flex-row gap-6 animate-[fadeIn_0.2s_ease-out] h-full">
      <div className="flex-1 min-w-0 flex flex-col h-full overflow-y-auto custom-scroll pr-2">
        ...
        {active.length === 0 ? <div className="text-center py-10 text-slate-400 font-bold text-xs bg-slate-50 rounded-xl border border-dashed border-slate-200">진행 중인 교재가 없습니다.</div> :
      ...
      <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm w-full flex flex-col h-full min-h-[400px]">
          <h3 className="text-[12px] font-black text-emerald-600 mb-3 border-b border-slate-100 pb-2 flex items-center gap-1.5"><span className="text-base">🏆</span> 완료된 교재 목록</h3>
          <div className="flex-1 overflow-y-auto custom-scroll space-y-2.5 pr-1">
             {completed.length === 0 ? <div className="text-center py-10 text-slate-400 font-bold text-[10px] bg-slate-50 rounded-lg">아직 완료된 교재가 없습니다.</div> :
               completed.map((b: any, i: number) => (
                 <div key={i} className="bg-emerald-50 border border-emerald-100 p-3 rounded-lg shadow-sm">
                    <div className="flex gap-1.5 mb-1.5"><span className="text-[8px] font-black text-slate-500 bg-white px-1 py-0.5 rounded border border-slate-200">{b.className}</span><span className="text-[8px] font-black text-emerald-600 bg-white px-1 py-0.5 rounded border border-emerald-200">{b.tbType}</span></div>
...
const RadarChart = ({ data }: { data: any[] }) => {
  if (!data || data.length < 3) {
    return <div className="text-center text-slate-400 text-[11px] font-bold py-10">데이터가 부족합니다.<br/>(방사 차트는 최소 3개 이상의 단원 데이터가 필요합니다.)</div>;
  }
...
function ExamTab({ examResults, schoolExams, categoryAnalysis, setIsSchoolExamModalOpen, deleteSchoolExam }: any) {
  const router = useRouter();
  const params = useParams();
  const studentId = params.id as string;
  ...
  const renderGrowthChart = () => {
    ...
    const maxScore = 100;
    const xStep = 85; 
    const dynamicWidth = Math.max(500, paddingX * 2 + (chartData.length - 1) * xStep + 40);
    ...
    <div className="space-y-6 animate-[fadeIn_0.2s_ease-out]">
      ...
      <div>
        <div className="flex justify-between items-center mb-3 border-b border-slate-100 pb-2 mt-2">
          ...
          <button onClick={() => setIsSchoolExamModalOpen(true)} className="px-2.5 py-1 bg-indigo-50 text-indigo-600 font-bold rounded text-[10px] hover:bg-indigo-100 transition-colors shadow-sm">+ 성적 추가</button>
        </div>
          ...
          {schoolExams.length === 0 ? <div className="col-span-full text-[11px] text-center py-4 text-slate-400 bg-slate-50 rounded-xl">학교 성적이 없습니다.</div> :
            schoolExams.map((se: any, idx: number) => (
              <div key={se.id || idx} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm relative group">
                ...
                <div className="flex justify-between items-end"><div className="font-black text-[11px] text-slate-700">{se.subject}</div><div className="text-[13px] font-black text-indigo-600">{se.score}점</div></div>
              </div>
            ))
          }
        </div>
    ...
    <div className="space-y-4 animate-[fadeIn_0.2s_ease-out]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {hwList.length === 0 ? <div className="col-span-full text-center py-10 text-slate-400 font-bold text-[11px] bg-slate-50 border border-slate-200 rounded-xl">배부된 과제가 없습니다.</div> :
          hwList.filter((hw: any) => unwrap(hw.homework_assignment)?.homework_title !== '[시스템] 수업 진도 완료 기록').map((hw: any, idx: number) => {
            const assign = unwrap(hw.homework_assignment); if (!assign) return null;
            ...
            if(hw.status.includes('제출')) statusCol = 'bg-blue-50 text-blue-600 border border-blue-200'; else if(hw.status.includes('완료')) statusCol = 'bg-emerald-50 text-emerald-600 border border-emerald-200'; else if(hw.status.includes('미')) statusCol = 'bg-rose-50 text-rose-500 border border-rose-100';
            return (
              <div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col justify-between shadow-sm hover:border-blue-300 transition-colors">
                <div className="flex justify-between items-start mb-2"><span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-black border border-slate-200">{unwrap(assign.textbook as any)?.title || '기타'}</span><span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${statusCol}`}>{hw.status}</span></div>
                ...
                <div className="flex flex-col gap-1 mt-1 mb-3 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                   <div className="flex justify-between items-center text-[10px]"><span className="font-bold text-slate-400">배부일: <span className="text-slate-600">{assign.created_at ? new Date(assign.created_at).toLocaleDateString().replace(/\.$/, '') : '-'}</span></span><span className="font-bold text-slate-400">마감일: <span className="text-rose-500">{assign.due_date ? new Date(assign.due_date).toLocaleDateString().replace(/\.$/, '') : '-'}</span></span></div>
                   <div className="flex justify-between items-center text-[10px]"><span className="font-bold text-slate-400">제출/확인: <span className="text-blue-600">{hw.status === '미제출' ? '-' : (hw.checked_at ? new Date(hw.checked_at).toLocaleDateString().replace(/\.$/, '') : '-')}</span></span></div>
                </div>
                <div className="flex justify-between items-end mt-auto pt-2 border-t border-slate-50"><div className="text-[10px] font-black text-slate-600">진행률 <span className="text-[#002864] ml-0.5">{compCount} / {qCount}</span></div></div>
              </div>
            );
          })
        }
...
function ClinicTab({ clinicList }: any) {
  return (
    <div className="space-y-4 animate-[fadeIn_0.2s_ease-out]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {clinicList.length === 0 ? <div className="col-span-full text-center py-10 text-slate-400 font-bold text-[11px] bg-slate-50 border border-slate-200 rounded-xl">클리닉/오답 배부 내역이 없습니다.</div> :
          clinicList.map((c: any, idx: number) => {
            let typeCol = 'bg-rose-50 text-rose-500 border border-rose-100';
            if(c.task_type === '유사') typeCol = 'bg-indigo-50 text-indigo-500 border border-indigo-100'; if(c.task_type === '쌍둥이') typeCol = 'bg-amber-50 text-amber-600 border border-amber-200';
            return (
              <div key={idx} className="bg-white p-3.5 rounded-xl border border-slate-200 flex flex-col justify-between shadow-sm hover:border-rose-300 transition-colors">
                <div className="flex justify-between items-start mb-2"><span className={`text-[9px] px-1.5 py-0.5 rounded font-black ${typeCol}`}>{c.task_type} 문항</span><span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${c.status==='해결'?'bg-emerald-50 text-emerald-600':'bg-slate-100 text-slate-500'}`}>{c.status}</span></div>
                <div className="text-[11px] font-black text-slate-700 mb-2">본교재 연결 문항: <span className="text-[#002864] ml-0.5">{unwrap(c.textbook_question as any)?.question_number || '-'}번</span></div>
...
function BillingTab({ billingList, setIsBillingModalOpen, setPayFormInit, setIsPaymentModalOpen }: any) {
  let total = 0, paid = 0, unpaid = 0;
  billingList.forEach((b: any) => { const amt = parseInt(b.amount) || 0; total += amt; if (b.status === "완납") paid += amt; else unpaid += amt; });
  return (
    <div className="space-y-4 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-4">
        <h3 className="font-extrabold text-slate-700 text-[13px]">월별 수납 및 청구 내역</h3>
        <button onClick={() => setIsBillingModalOpen(true)} className="px-3 py-1.5 bg-emerald-600 text-white font-bold rounded-lg text-[11px] shadow-sm hover:bg-emerald-700 transition-colors flex items-center gap-1">+ 수동 청구</button>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex flex-col items-center justify-center"><span className="text-[10px] font-bold text-slate-500 mb-0.5">총 청구 금액</span><span className="text-sm font-black text-slate-800">{total.toLocaleString()}원</span></div>
        <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 shadow-sm flex flex-col items-center justify-center"><span className="text-[10px] font-bold text-rose-500 mb-0.5">미납 금액</span><span className="text-sm font-black text-rose-600">{unpaid.toLocaleString()}원</span></div>
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 shadow-sm flex flex-col items-center justify-center"><span className="text-[10px] font-bold text-emerald-600 mb-0.5">납부 완료 금액</span><span className="text-sm font-black text-emerald-700">{paid.toLocaleString()}원</span></div>
      </div>
      <div className="space-y-2">
        {billingList.length === 0 ? <div className="text-center py-10 text-slate-400 font-bold border border-slate-200 rounded-xl bg-slate-50 text-[11px]">청구 내역이 없습니다.</div> :
          billingList.map((bill: any, idx: number) => (
            <div key={bill.billing_id || bill.id || idx} className={`bg-white p-3 rounded-xl border ${bill.status === '완납' ? 'border-slate-200' : 'border-rose-300'} shadow-sm flex items-center justify-between hover:bg-slate-50 transition-colors`}>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-slate-100 flex flex-col items-center justify-center border border-slate-200 shrink-0 shadow-inner"><span className="text-[8px] text-slate-500 font-bold">{bill.billing_month.split('-')[0]}</span><span className="text-[12px] font-black text-slate-700 leading-none">{bill.billing_month.split('-')[1]}월</span></div>
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className="text-[12px] font-extrabold text-[#002864]">{unwrap(bill.class as any)?.name || '미배정'}</span>
                  </div>
