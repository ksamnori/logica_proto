...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    ...
    if (activeTab === "exam") { loadExamResults(); loadSchoolExams(); }
  ...
  const loadExamResults = async () => {
    const { data } = await supabase.from("student_exam_result").select("original_score, final_score, class_avg, submitted_at, exam_assignment(exam_master(title))").eq("student_id", studentId).not("final_score", "is", null).order("submitted_at", { ascending: false }).limit(10);
...
function ExamTab({ examResults, schoolExams, setIsSchoolExamModalOpen, deleteSchoolExam, examChart }: any) {
  return (
    <div className="space-y-6 animate-[fadeIn_0.2s_ease-out]">
      {examChart}
      <div>
        <div className="flex justify-between items-center mb-3 border-b border-slate-100 pb-2"><h3 className="font-extrabold text-slate-700 text-[13px] flex items-center gap-1.5"><span className="w-1 h-3 bg-emerald-500 rounded-full"></span>자체 테스트 기록</h3></div>
          ...
          {examResults.length === 0 ? <div className="col-span-full text-[11px] text-slate-400 text-center py-4 bg-slate-50 rounded-xl">테스트 기록이 없습니다.</div> :
             examResults.slice().reverse().map((ex: any, i: number) => (
               <div key={i} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
                 ...
                 <div className="flex justify-between items-end mt-2"><div className="text-[9px] text-slate-400 font-bold">반 평균 {ex.class_avg || '-'}</div><div className="text-sm font-black text-[#002864]">{ex.final_score !== null ? `${ex.final_score}점` : '-'}</div></div>
               </div>
             ))
          }
        </div>
