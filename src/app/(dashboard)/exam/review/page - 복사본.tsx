...
// 💡 1. [UI 성능 최적화] 내부 함수로 있던 컴포넌트를 외부로 분리하고 memo 적용
...
function ReviewContent() {
  ...
  const [isDeletingReport, setIsDeletingReport] = useState(false); // 💡 삭제 로딩 상태 추가
  ...
  const loadHomeworkResults = async () => {
    try {
      ...
      // 💡 [방어 1] Set을 이용해 중복된 문항 ID로 답안이 이중 생성되는 것 원천 차단
      const uniqueTqIds = Array.from(new Set(tqIds));
      const missingTqIds = uniqueTqIds.filter(id => !answers.find((a: any) => String(a.tq_id) === String(id)));
      if (missingTqIds.length > 0) {
        const newAns = missingTqIds.map(id => ({ homework_id: homeworkId, tq_id: id, student_id: sId, is_correct: false, grading_code: null, earned_score: 0 }));
        await supabase.from('student_homework_answer').insert(newAns);
        
        const { data: refetched } = await supabase.from('student_homework_answer').select('*').eq('homework_id', homeworkId).eq('student_id', sId);
        if (refetched) answers = refetched;
      }

      // 💡 [방어 2] 화면 데이터 로드 시 중복 답안 완벽 필터링 (채점된 데이터 우선)
      ...
      tqData.forEach((tq: any, idx: number) => {
        ...
        // 💡 [핵심 버그 수정] 문제 그룹핑 시 page 대신 실제 문제 번호를 우선 사용합니다.
  ...
  const loadExamResults = async () => {
    try {
      ...
      const missingQIds = uniqueQIds.filter(qid => !answers.find((a:any) => String(a.question_id) === String(qid)));
      if (missingQIds.length > 0) {
        const newAns = missingQIds.map(qid => ({ exam_assignment_id: assignmentId, question_id: qid, student_id: sId, student_input: '미입력', is_correct: false, grading_code: null, earned_score: 0 }));
        await supabase.from('student_answer').insert(newAns);
        
        const { data: refetched } = await supabase.from('student_answer').select('*').eq('exam_assignment_id', assignmentId).eq('student_id', sId);
        if (refetched) answers = refetched;
      }
      ...
      items.forEach(item => {
        ...
        // 💡 [핵심 버그 수정] 문제 그룹핑 시 page나 pdf source 대신 실제 문제 번호를 우선 사용합니다.
      ...
      groups.forEach((g: any, index: number) => {
        ...
        // 💡 [핵심 버그 수정] 화면 순서가 밀려도 '실제 문제 번호'를 추출하여 정확한 배점을 찾아옵니다.
  ...
  const saveOnlyGrades = async () => {
    ...
    try {
      if (isHomeworkMode) {
        ...
        // 💡 [방어 3] 시험 총점 계산 시에도 중복 가짜 0점 데이터 무시 로직 적용
  ...
  const triggerReportGeneration = async () => {
    ...
    try {
      // 💡 [방어 3] 리포트용 점수 산출 시에도 중복 가짜 0점 데이터 무시
      ...
      // 💡 [확실한 덮어쓰기] 중복 꼬임 방지를 위해 무조건 싹 지우고 깨끗하게 다시 생성합니다.
  ...
  const handleDeleteReport = async () => {
    ...
    try {
      ...
      setShowReportBtn(false); // 💡 리포트 삭제 후 즉시 열기 버튼 숨김
      ...
      <main className="flex-1 overflow-y-auto custom-scroll p-4 sm:p-8 relative">
        <div className="max-w-[1500px] w-full mx-auto relative pb-24">
          ...
          <div className="bg-[#002864] text-white p-6 rounded-t-2xl shadow-md flex justify-between items-center no-print">
            <div>
              <button onClick={() => router.back()} className="text-blue-300 hover:text-white flex items-center gap-1 font-bold text-sm mb-2 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg> 돌아가기
              </button>
            ...
            <div className="text-right flex items-center gap-4">
              {/* 💡 캐시 무력화 파라미터 추가 */}
              {showReportBtn && (
                <button onClick={() => window.open(`/admission/report?assignment_id=${assignmentId}&t=${Date.now()}`, '_blank')} className="bg-emerald-500 hover:bg-emerald-600 text-white font-extrabold px-5 py-2.5 rounded-xl shadow-lg flex items-center gap-2 transition-colors">
                  📊 진단 분석 레포트 열기
                </button>
              )}
          ...
          <div className="bg-white shadow-md rounded-b-2xl overflow-hidden overflow-x-auto custom-scroll no-print">
                ...
                {resultData.length === 0 ? <tr><td colSpan={7} className="p-12 text-center text-slate-500 font-bold text-lg">데이터 동기화 중입니다...</td></tr> : 
                  resultData.map((g, gIdx) => {
                    ...
                    return g.items.map((row: any, subIdx: number) => {
                      ...
                      let sAnswerHTML = <div dangerouslySetInnerHTML={{ __html: formatMathTextForWeb(a.student_input || "미입력") }} />;
                      ...
                      return (
                        <tr key={a.answer_id} className={`transition-colors border-b border-slate-200 ${rowBg}`}>
                          ...
                          <td className="p-3 font-bold text-emerald-700 bg-white/50 text-[13px] leading-tight break-all">
                            <span dangerouslySetInnerHTML={{ __html: qPrefix + qAnswer }} />
                          </td>
                          <td className="p-3 font-bold text-blue-700 bg-white/50 text-[13px] leading-tight break-all">
                            <div className="flex items-center justify-between gap-2">
                              <div className="min-w-0 relative">{sAnswerHTML}</div>
                              {wrongLog.length > 0 && (
            ...
            <div className="bg-slate-50 border-t border-slate-200 p-6 flex justify-end items-center sticky bottom-0 z-10 gap-3 shadow-[0_-10px_20px_rgba(0,0,0,0.03)]">
              ...
              {headerInfo.type === '입학테스트' && showReportBtn && (
                <button onClick={() => window.open(`/admission/report?assignment_id=${assignmentId}&t=${Date.now()}`, '_blank')} className="bg-emerald-500 hover:bg-emerald-600 text-white font-extrabold text-lg py-3.5 px-6 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5">
                  3️⃣ 진단 리포트 열기 📊
                </button>
              )}
              {/* 💡 리포트 삭제 버튼 추가 */}
              ...
              <div className="p-6 overflow-y-auto custom-scroll flex-1 bg-slate-50 space-y-6">
                  ...
                  {modalQ.items.map((row: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="mb-4">
                        <div dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.question) }} className="math-text text-slate-700 font-medium" />
                ...
                <div className="bg-blue-50 p-5 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  {modalQ.items.map((row: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="mb-4">
                        <div dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.answer) }} className="math-text text-blue-700 font-bold text-lg" />
                    ...
                    {modalQ.items.map((row: any, idx: number) => {
                      ...
                      return (
                        <div key={idx} className="space-y-4">
                          {q.step_1_concept && <div><strong className="text-indigo-600 block mb-1.5 flex items-center gap-1"><span className="bg-indigo-100 text-indigo-600 px-1.5 rounded text-xs">Step 1</span> 핵심 개념</strong><div className="math-text bg-slate-50 p-3 rounded-lg border border-slate-100" dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.step_1_concept) }} /></div>}
                          {q.step_2_approach && <div><strong className="text-emerald-600 block mb-1.5 flex items-center gap-1"><span className="bg-emerald-100 text-emerald-600 px-1.5 rounded text-xs">Step 2</span> 접근 방법</strong><div className="math-text bg-slate-50 p-3 rounded-lg border border-slate-100" dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.step_2_approach) }} /></div>}
                          {q.step_3_process && <div><strong className="text-amber-600 block mb-1.5 flex items-center gap-1"><span className="bg-amber-100 text-amber-600 px-1.5 rounded text-xs">Step 3</span> 풀이 과정</strong><div className="math-text bg-slate-50 p-3 rounded-lg border border-slate-100" dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.step_3_process) }} /></div>}
                          {q.step_4_conclusion && <div><strong className="text-rose-600 block mb-1.5 flex items-center gap-1"><span className="bg-rose-100 text-rose-600 px-1.5 rounded text-xs">Step 4</span> 결론 도출</strong><div className="math-text bg-slate-50 p-3 rounded-lg border border-slate-100" dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.step_4_conclusion) }} /></div>}
                          {q.explanation && <div><strong className="text-slate-600 block mb-1.5">[일반 해설]</strong><div className="math-text bg-slate-50 p-3 rounded-lg border border-slate-100" dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.explanation) }} /></div>}
                          {q.solution && <div><strong className="text-slate-600 block mb-1.5">[일반 풀이]</strong><div className="math-text bg-slate-50 p-3 rounded-lg border border-slate-100" dangerouslySetInnerHTML={{ __html: pre + formatMathTextForWeb(q.solution) }} /></div>}
                        </div>
                      );
                    })}
                  </div>
                  ...
                  return (
                    <div key={i} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                      ...
                      {entry.input?.startsWith('data:image') ? (
                        ...
                        <p dangerouslySetInnerHTML={{ __html: formatMathTextForWeb(entry.input || '') }} className="math-text text-slate-700 font-bold mt-1" />
                      )}
                    </div>
                  );
                })}
...
// Next.js Search Params 사용을 위한 래핑 컴포넌트
