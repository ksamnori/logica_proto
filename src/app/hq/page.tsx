      ...
      const roomsWithData = await Promise.all((data || []).map(async (r: any) => {
        ...
        if (r.internal_chat_room?.room_type === 'DIRECT') {
          ...
          if (otherMember?.instructor_id) {
            const { data: instData } = await supabase.from('instructor').select('name, profile_image_url').eq('instructor_id', otherMember.instructor_id).maybeSingle();
            if (instData) {
    ...
    if (selectedInstIds.length === 1) {
      ...
      if (target) {
        try {
          ...
          if (!rId) {
            const { data: newRoom } = await supabase.from('internal_chat_room').insert({ room_type: 'DIRECT', title: `${target.name} 선생님`, created_by: instId }).select().single();
            if (newRoom) {
              rId = newRoom.room_id;
          ...
          if (rId) openRoom(rId, `${target.name} 선생님`);
                                        ...
                                        return (
                                          <div key={inst.instructor_id} onClick={() => toggleInstSelection(inst.instructor_id)} className={`bg-white p-3 rounded-lg border shadow-sm cursor-pointer transition-all flex items-center gap-3 ${isSelected ? 'border-slate-800 bg-slate-50' : 'border-slate-200 hover:border-slate-400'}`}>
                                            ...
                                            <div className="flex-1">
                                              <div className="font-bold text-slate-800 text-sm">{inst.name}</div>
                                              <div className="text-[11px] text-slate-500 font-bold mt-0.5">{inst.position || '직원'}</div>
                                            </div>
                                          </div>
                                        )
                                      })}
