        ...
        return (
            <div key={cardKey} className={`w-full h-full ${theme.bg} rounded-[2rem] p-8 md:p-10 text-white shadow-xl relative overflow-hidden group flex flex-col transition-all duration-300 ${isLocked ? 'grayscale-[60%] opacity-80' : ''}`}>
                ...
                <div className="relative z-10 shrink-0">
                    ...
                    <div className="absolute top-0 right-0 flex flex-col items-end gap-2 z-20 w-auto max-w-[55%] md:max-w-[300px]">
                        <div className="flex items-center gap-2">
                            {isSelectedItemWaiting && initialScore !== null && (
                                <span className="text-sm md:text-base font-bold text-white bg-black/20 px-3.5 py-1.5 rounded-lg whitespace-nowrap shadow-sm">가채점: {initialScore}점</span>
                            )}
                            {!isBoxDone && !isSelectedItemWaiting && qCount > 0 && <span className={`text-sm md:text-base font-bold ${isLocked ? 'text-white/70 bg-black/20' : theme.textColor + ' bg-black/10'} px-3.5 py-1.5 rounded-lg whitespace-nowrap shadow-sm`}>남은 문제: {selectedItem.remain || qCount}</span>}
                        </div>
                    ...
                    <p className={`text-sm md:text-base font-medium mt-0 mb-1.5 leading-snug ${isLocked ? 'text-white/70' : theme.textColor} pr-[60px] md:pr-[10px]`}>
        ...
        return (
            <div key={cardKey} className={`w-full h-full ${theme.bg} rounded-[2rem] p-8 md:p-10 text-white shadow-xl relative overflow-hidden group flex flex-col justify-between transition-all duration-300 ${isLocked ? 'grayscale-[60%] opacity-80' : ''}`}>
                ...
                <div className="relative z-10 shrink-0">
                    ...
                    <div className="absolute top-0 right-0 flex flex-col items-end gap-2 z-20 w-auto max-w-[55%] md:max-w-[300px]">
                        {!isBoxDone && qCount > 0 && <span className={`text-sm md:text-base font-bold ${isLocked ? 'text-white/70 bg-black/20' : theme.textColor + ' bg-black/10'} px-3.5 py-1.5 rounded-lg whitespace-nowrap shadow-sm`}>남은 문제: {selectedItem.remain || qCount}</span>}
                    ...
                    <h3 className={`text-[26px] md:text-[32px] lg:text-[36px] font-black mb-1.5 leading-tight ${isLocked ? 'text-white/90' : ''} pr-[80px] md:pr-[120px]`}>미완료 과제 클리닉</h3>
                    <p className={`text-sm md:text-base font-medium mt-0 mb-1.5 leading-snug ${isLocked ? 'text-white/70' : theme.textColor} pr-[60px] md:pr-[10px]`}>
