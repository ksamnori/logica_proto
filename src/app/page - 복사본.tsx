...
export default function LoginPage() {
  const router = useRouter(); // 💡 선언된 Next.js 라우터 적극 활용
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🚨 [진단용 알림창] 여기서 시스템이 누구를 찾았는지 정확히 검문합니다! 복사해서 붙여넣으세요.
      alert(`[시스템 진단]\n내가 타이핑한 아이디: ${loginId}\nDB에서 꺼내온 사람: ${instructorData.name}`);
      // -------------------------------------------------------------
      ...
      // 💡 직행(스무스한 이동)을 방해하는 alert는 주석 처리하거나 지우는 것이 UX에 좋습니다.
      // alert(`환영합니다! ${instructorData.name} ${instructorData.position || '선생'}님.`);
      ...
      const isManager = role === 'PRINCIPAL' || role === 'MANAGER' || position.includes('원장') || position.includes('실장');

      // 🌟 [핵심 변경] window.location.href 대신 Next.js의 router.replace 사용
      if (isSuperAdmin) {
